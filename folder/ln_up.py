#!/usr/bin/env python3
"""
LiDex - Novel Sync (licensed_books.sql -> Neon DB)
...
import sys, os, re, json, time, argparse, psycopg2
from urllib.parse import quote
from datetime import datetime, timezone, date
from collections import OrderedDict

ORIGINAL_WEB_DIR = r'C:\Users\ADMIN\Desktop\Web'
if ORIGINAL_WEB_DIR not in sys.path:
    sys.path.insert(0, ORIGINAL_WEB_DIR)

DEFAULT_XLSX_PATH = r'C:\Users\ADMIN\Desktop\Web\Supabase pre\series_table_import_v5.xlsx'
...
from sync_common import fetch_json
...
def build_series_rows_from_xlsx(raw_rows):
    rows = []
    workbook_id_to_series_key = {}

    for raw in raw_rows:
        workbook_id = raw.get('id')
        item_type = _clean_title(raw.get('item_type')) or 'novel'
        title = _clean_title(raw.get('title') or raw.get('title_vi'))
        title_vi = _clean_title(raw.get('title_vi')) or title
        slug = _clean_title(raw.get('slug')) or _slugify(title)
        if not title:
            continue
        if not slug:
            continue
        if workbook_id is not None:
            workbook_id_to_series_key[workbook_id] = {
                'item_type': item_type,
                'title': title,
                'title_vi': title_vi,
                'slug': slug,
        ...
        rows.append({
            ...
            'title':          title,
            'title_vi':       title_vi,
            'title_native':   _clean_title(raw.get('title_native')),
            'title_english':  _clean_title(raw.get('title_english')),
            'slug':           slug,
            ...
            'author':         _first_raw(raw, 'author', 'Author', 'authors', 'Authors', 'writer', 'Writer'),
            'artist':         _first_raw(raw, 'artist', 'Artist', 'artists', 'Artists', 'illustrator', 'Illustrator', 'illustrators', 'Illustrators'),
    ...
    return rows, workbook_id_to_series_key
...
def _flush(rows, table, conflict, label=None):
    ...
    conflict_targets = [c.strip() for c in conflict.split(',') if c.strip()]
    conflict_clause = f"({', '.join(conflict_targets)})"
    ...
    try:
        for i in range(0, len(rows), batch_size):
            ...
            values = []
            for r in chunk:
                val_tuple = tuple(r[k] for k in keys)
                values.append(val_tuple)
            cursor.executemany(query, values)
            total += len(chunk)
        conn.commit()
    except Exception as e:
        print(f'  {label} insert error: {e}')
        conn.rollback()
    finally:
        cursor.close()
        conn.close()
...
def _resolve_series_by_slug(slugs):
    ...
    try:
        conn = psycopg2.connect(db_url)
        cursor = conn.cursor()
        cursor.execute("SELECT id, slug FROM series WHERE slug = ANY(%s)", (unique,))
        rows = cursor.fetchall()
        for row in rows:
            id_val, slug_val = row
            if slug_val:
                id_map[slug_val] = id_val
    except Exception as e:
...
def _resolve_series_by_anilist(al_ids):
    ...
    try:
        conn = psycopg2.connect(db_url)
        cursor = conn.cursor()
        cursor.execute("SELECT id, anilist_id FROM series WHERE anilist_id = ANY(%s)", (unique,))
        rows = cursor.fetchall()
        for row in rows:
            id_val, al_id_val = row
            if al_id_val:
                id_map[al_id_val] = id_val
    except Exception as e:
...
def _fetch_novel_series_lookup():
    ...
    try:
        conn = psycopg2.connect(db_url)
        cursor = conn.cursor()
        cursor.execute("SELECT id, slug, title, title_vi FROM series WHERE item_type = 'novel'")
        rows = cursor.fetchall()
        for row in rows:
            id_val, slug_val, title_val, title_vi_val = row
            if slug_val:
                lookup['by_slug'][slug_val.strip().lower()] = id_val
            if title_vi_val:
                lookup['by_title_vi'][title_vi_val.strip()] = id_val
                title_vi_key = _title_lookup_key(title_vi_val)
                if title_vi_key:
                    lookup['by_title_vi_norm'][title_vi_key] = id_val
            if title_val:
                lookup['by_title'][title_val.strip()] = id_val
                title_key = _title_lookup_key(title_val)
                if title_key:
                    lookup['by_title_norm'][title_key] = id_val
    except Exception as e:
...
def reset_sequences():
    try:
        conn = psycopg2.connect(db_url)
        cursor = conn.cursor()
        cursor.execute("SELECT setval(pg_get_serial_sequence('series', 'id'), COALESCE(MAX(id), 1)) FROM series")
        cursor.execute("SELECT setval(pg_get_serial_sequence('volumes', 'id'), COALESCE(MAX(id), 1)) FROM volumes")
        cursor.execute("SELECT setval(pg_get_serial_sequence('series_links', 'id'), COALESCE(MAX(id), 1)) FROM series_links")
        conn.commit()
    except Exception as e:
...
def sync_volume_publishers():
    print(f'\n  Synchronizing volume publisher IDs from ln_series_ranking...')
    try:
        conn = psycopg2.connect(db_url)
        cursor = conn.cursor()
        query = """
            ...
            FROM series s
            JOIN ln_series_ranking r ON (
                LOWER(TRIM(s.title)) = LOWER(TRIM(r.series_title)) 
                OR LOWER(TRIM(s.title_vi)) = LOWER(TRIM(r.series_title))
            )
            JOIN publishers p ON (
                LOWER(TRIM(p.name)) = LOWER(TRIM(r.publisher))
                OR LOWER(TRIM(p.name_vi)) = LOWER(TRIM(r.publisher))
            )
        ...
        conn.commit()
        print(f"  Successfully synchronized {updated} volume publisher IDs.")
    except Exception as e:
        print(f"  Warning: Failed to synchronize volume publishers: {e}")
    finally:
        if 'cursor' in locals():
            cursor.close()
        if 'conn' in locals():
...
def run_xlsx_sync(xlsx_path, dry_run=False, skip_existing=False):
    ...
    series_rows, workbook_id_to_series_key = build_series_rows_from_xlsx(raw_series)
    ...
    if skip_existing:
        ...
        existing_titles = {
            row['title']: (
                novel_lookup['by_title'].get(row.get('title'))
                or novel_lookup['by_title_norm'].get(_title_lookup_key(row.get('title')))
                or novel_lookup['by_title_vi'].get(row.get('title_vi'))
                or novel_lookup['by_title_vi_norm'].get(_title_lookup_key(row.get('title_vi')))
            )
            for row in series_rows
            if row.get('title')
        }
        existing_titles = {title: sid for title, sid in existing_titles.items() if sid}
        print(f'  Found {len(existing_titles)} existing series by title')

    series_to_write = [
        row for row in series_rows
        if not skip_existing or row.get('title') not in existing_titles
    ]

    if series_to_write:
        print(f'\n  series ({len(series_to_write)} rows; upsert by item_type,title)...')
        _flush(series_to_write, 'series', 'item_type,title', label='series')
    else:
        print(f'\n  series: 0 rows to write (all already exist)')
    ...
    workbook_id_to_db_id = {
        workbook_id: (
            novel_lookup['by_title'].get(series_key.get('title'))
            or novel_lookup['by_title_norm'].get(_title_lookup_key(series_key.get('title')))
            or novel_lookup['by_title_vi'].get(series_key.get('title_vi'))
            or novel_lookup['by_title_vi_norm'].get(_title_lookup_key(series_key.get('title_vi')))
            or slug_to_id.get(series_key.get('slug'))
            or novel_lookup['by_slug'].get(series_key.get('slug'))
        )
        for workbook_id, series_key in workbook_id_to_series_key.items()
    }
    workbook_id_to_db_id = {
        workbook_id: sid
        for workbook_id, sid in workbook_id_to_db_id.items()
    ...
    unresolved = [
        workbook_id for workbook_id in workbook_id_to_series_key
    ...
    print(f'\n  Synchronizing volume publishers...')
    sync_volume_publishers()
    ...
    print(f'  Series:  {len(series_rows)} total, {len(series_to_write)} written')
